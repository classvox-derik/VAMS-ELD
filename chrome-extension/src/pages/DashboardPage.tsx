...
export default function DashboardPage() {
    ...
    const [stats, setStats] = useState({ total: 0, byLevel: {} as Record<string, number>, byGrade: {} as Record<number, number>, byHomeroom: {} as Record<string, number> });
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        if (!supabase) return;
        (async () => {
            const { data: students } = await supabase.from("students").select("grade, el_level, homeroom");
            if (!students || students.length === 0) {
                setStats({ total: 0, byLevel: { Emerging: 0, Expanding: 0, Bridging: 0 }, byGrade: {}, byHomeroom: {} });
                setLoading(false);
                return;
            }
            const byLevel: Record<string, number> = { Emerging: 0, Expanding: 0, Bridging: 0 };
            const byGrade: Record<number, number> = {};
            const byHomeroom: Record<string, number> = {};
            for (const s of students) {
                if (s.el_level in byLevel) byLevel[s.el_level as string]++;
                byGrade[s.grade] = (byGrade[s.grade] || 0) + 1;
                if (s.homeroom) byHomeroom[s.homeroom] = (byHomeroom[s.homeroom] || 0) + 1;
            }
            setStats({ total: students.length, byLevel, byGrade, byHomeroom });
            setLoading(false);
        })();
    }, [supabase]);

    if (loading) return <div className="p-6 text-center text-muted-foreground">Loading dashboard...</div>;

    return (
        <div className="space-y-8 p-4 md:p-6">
            <div>
                <h1 className="scaffold-heading">Dashboard</h1>
                <p className="scaffold-description mt-1">Here's an overview of your ELD scaffolding workspace.</p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-6 xl:grid-cols-3">
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">Students by EL Level</CardTitle>
                        <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-eld-almond-silk/30">
                            <Users className="h-6 w-6 text-eld-space-indigo dark:text-eld-almond-silk" />
                        </div>
                    </CardHeader>
                                ...
                                return (
                                    <div key={level} className="space-y-1">
                                        <div className="flex items-center justify-between text-sm">
                                            <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${levelStyles[level]}`}>{level}</span>
                        ...
                        <div className="space-y-3">
                            {[5, 6, 7, 8].map((grade) => (
                                <div key={grade} className="flex items-center justify-between text-sm">
                                    ...
                                    <span className="font-medium">{stats.byGrade[grade] || 0}</span>
                                </div>
                            ))}
                        </div>
                    </CardContent>
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">Students by Homeroom</CardTitle>
                        <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-eld-almond-silk/30">
                    ...
                    <CardContent>
                        <div className="space-y-3">
                            {Object.entries(stats.byHomeroom).sort(([a], [b]) => a.localeCompare(b)).map(([homeroom, count]) => (
                                <div key={homeroom} className="flex items-center justify-between text-sm">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-6">
                <Card className="flex flex-col items-center justify-center p-6 text-center">
                    ...
                    <p className="text-xs text-muted-foreground mb-4">Create differentiated assignments for your {stats.total} students</p>
